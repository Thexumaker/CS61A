    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) and not insect.is_container and not self.ant.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        first_place = self.place
        while first_place != hive:
            if  counter <= self.max_range and counter >= self.min_range and first_place.bees:
                return random_or_none(first_place.bees)
            else:
                counter +=1
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if other.is_container == True and self.contained_ant:
            return False
        else:
    ...
    def action(self, colony):
        ...
        def double(place):
            if not type(place.ant) == QueenAnt:
                if place.ant  not in self.buffed and place.ant is not None:
