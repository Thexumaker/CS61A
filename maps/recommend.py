...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        z = group_by_centroid(restaurants,old_centroids)
        centroids = [find_centroid(cluster) for cluster in z]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    "*** YOUR CODE HERE ***"
    sx = sum([(x -mean(xs))**2 for x in xs])
    sy = sum([(y - mean(ys)) **2 for y in ys])
